...
local function parse_ref_from_line(line, is_json)
	...
	if is_json then
		...
		raw = line:match("%$ref%s*:%s*[\"']([^\"']+)[\"']") or line:match("%$ref%s*:%s*([^%s#'\"]+[^'\",%s]*)")
	end
...
function M.goto_definition()
	...
	if not file then
		vim.notify("openapi-navigator: file not found: " .. (ref.file or "<current>"), vim.log.levels.ERROR)
		return false
	end
	...
	if not pos then
		vim.notify("openapi-navigator: could not resolve pointer: " .. ref.pointer, vim.log.levels.ERROR)
		return false
	end
