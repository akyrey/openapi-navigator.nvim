...
local function expand_refs(block_lines, source_file, depth, max_depth)
	...
	for _, line in ipairs(block_lines) do
		...
		if ref then
			...
			local target_file = ref.file and vim.fn.resolve(vim.fn.fnamemodify(source_file, ":h") .. "/" .. ref.file)
				or source_file
...
function M.show()
	...
	if not target_file then
		vim.notify("openapi-navigator: hover: file not found: " .. (ref.file or "<current>"), vim.log.levels.WARN)
		return
	end
